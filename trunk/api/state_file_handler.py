...
import datetime
import traceback
from service.query_state_sales_service import query
...
class StateFileHandler(web.RequestHandler):
    """
    页面下载表格，后台导出文件流
    """
    ...
    def get(self):
        ...
        try:
            ...
            self.set_header('Content-Disposition', 'attachment; filename=state_sales.xls')
    ...
    def file_exits(self):
        ...
        :return:
        """
        my_file = '/opt/powercloud/state_sales.xls'
    ...
    def write_xls(self, data_array):
        ...
        try:
            ...
            sheet.write(0, 0, 'State ', xlwt.easyxf('font: height 250, ' 'bold off'))
            ...
            #  对查询好的字典信息按照州名降序排列
            data_array = sorted(data_array, key=lambda data_array: data_array['state'])
